...
def train2_org(model,train_loader,valid_loader1,valid_loader2,metric_crit,optimizer,scheduler,num_epochs,eval_every,num_class,device,name):
    IOU_list = []
    best_IOU = 1
    global_step = 0
    ...
    for epoch in range(num_epochs):
        ...
        try:
            ...
            print("Model results in collapse",str(e)) # if the collapse to 0 then, the result cannot be printed
        print('dist_threshold:',dist_threshold,'overlap:',overlap,'IOU:',IOU)
        plt.clf()
        IOU_list.append(IOU)
        if IOU < best_IOU:
